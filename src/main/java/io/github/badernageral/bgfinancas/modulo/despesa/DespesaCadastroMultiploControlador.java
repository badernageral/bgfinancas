...
public final class DespesaCadastroMultiploControlador implements Initializable, Controlador, ControladorFiltro {
    ...
    @FXML private Button cupomFiscal;
    ...
    private void getCupomFiscal(){
        ModalCupomFiscalControlador janela = Janela.abrir(Despesa.FXML_MODAL_CUPOM_FISCAL, TITULO, true);
        Kernel.palcoModal.showAndWait();
        List<Despesa> resultado = janela.getResultado();
        if(!resultado.isEmpty()){
            for(Despesa d : resultado){
                tabelaLista.getItems().add(d);
            }
        }
        calcularValorTotal();
    }
    ...
    private void limparFormulario(){
        ...
        int c = conta.getSelectionModel().getSelectedIndex();
        acaoFiltrar(true);
        conta.getSelectionModel().select(c);
    }
