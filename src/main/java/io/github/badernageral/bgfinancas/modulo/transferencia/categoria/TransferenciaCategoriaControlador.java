...
public final class TransferenciaCategoriaControlador implements Initializable, Controlador {
       
    private final String TITULO = idioma.getMensagem("transferencias")+" - "+idioma.getMensagem("categorias");
    
    @FXML private CenaPadrao cenaController;
    private ObservableList<TransferenciaCategoria> itens;
    private Tabela<TransferenciaCategoria> tabela = new Tabela<>();
    private TableView<TransferenciaCategoria> tabelaLista = new TableView<>();
    ...
    public void acaoFiltrar(Boolean animacao){
        tabelaLista.setItems(new TransferenciaCategoria().setNome(cenaController.getFiltro().getText()).listar());
        if(animacao){
    ...
    public void acaoCadastrar(int botao) {
        TransferenciaCategoriaFormularioControlador Controlador = Janela.abrir(TransferenciaCategoria.FXML_FORMULARIO, TITULO);
    ...
    public void acaoExcluir(int botao) {
        ...
        if(Validar.exclusao(itens, cenaController.getBotaoExcluir())){
            try {
                for(TransferenciaCategoria c : itens){
                    c.excluir();
                }
    ...
    public void acaoAjuda() {
        ...
        Ajuda.getInstance().capitulo(Posicao.CENTRO, idioma.getMensagem("tuto_trans_cat_1"));
