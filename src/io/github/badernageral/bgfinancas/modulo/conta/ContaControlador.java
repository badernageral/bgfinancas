...
package io.github.badernageral.bgfinancas.modulo.conta;
...
public final class ContaControlador implements Initializable, Controlador {
       
    private final String TITULO = idioma.getMensagem("contas");
    
    @FXML private CenaPadrao cenaController;
    private ObservableList<Conta> itens;
    private final Tabela<Conta> tabela = new Tabela<>();
    private final TableView<Conta> tabelaLista = new TableView<>();;
    ...
    public void acaoFiltrar(Boolean animacao){
        tabelaLista.setItems(new Conta().setNome(cenaController.getFiltro().getText()).listar());
        if(animacao){
    ...
    public void acaoAjuda() {
        ...
        Ajuda.getInstance().capitulo(Posicao.CENTRO, idioma.getMensagem("tuto_conta_1"));
        Ajuda.getInstance().capitulo(Posicao.CENTRO, idioma.getMensagem("tuto_conta_2"));
        Ajuda.getInstance().capitulo(Posicao.CENTRO, idioma.getMensagem("tuto_conta_3"));
