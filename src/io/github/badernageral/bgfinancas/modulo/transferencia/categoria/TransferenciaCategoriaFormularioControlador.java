...
package io.github.badernageral.bgfinancas.modulo.transferencia.categoria;
...
import io.github.badernageral.bgfinancas.modulo.transferencia.item.TransferenciaItemFormularioControlador;
import javafx.scene.control.TitledPane;

public final class TransferenciaCategoriaFormularioControlador implements Initializable, ControladorFormulario {
    ...
    private TransferenciaItemFormularioControlador controlador = null;
    private TransferenciaCategoria modelo;
    
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        formulario.setText(idioma.getMensagem("transferencia")+" - "+idioma.getMensagem("categoria"));
    ...
    public void cadastrar(TransferenciaItemFormularioControlador controlador){
        acao = Acao.CADASTRAR;
        this.controlador = controlador;
        botaoController.setTextBotaoFinalizar(idioma.getMensagem("cadastrar"));
    }
    
    public void alterar(TransferenciaCategoria modelo){
        this.modelo = modelo;
        acao = Acao.ALTERAR;
    ...
    public void acaoFinalizar(){
        if(validarFormulario()){
            if(acao == Acao.CADASTRAR){
                TransferenciaCategoria categoria = new TransferenciaCategoria(null, nome.getText());
                categoria.cadastrar();
                if(controlador==null){
                    ...
                    categoria = new TransferenciaCategoria().setNome(categoria.getNome()).consultar();
                    controlador.selecionarComboCategoria(1, categoria);
                }
            }else if(acao == Acao.ALTERAR){
                TransferenciaCategoria categoria = new TransferenciaCategoria(modelo.getIdCategoria(), nome.getText());
                categoria.alterar();
                Kernel.principal.acaoTransferenciaCategoria();
                Janela.showTooltip(Status.SUCESSO, idioma.getMensagem("operacao_sucesso"), Duracao.CURTA);
                Animacao.fadeInOutClose(formulario);
