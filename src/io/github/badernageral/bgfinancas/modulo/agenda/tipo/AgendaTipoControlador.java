...
package io.github.badernageral.bgfinancas.modulo.agenda.tipo;
...
public final class AgendaTipoControlador implements Initializable, Controlador {
       
    private final String TITULO = idioma.getMensagem("lembretes")+" - "+idioma.getMensagem("tipo");
    
    @FXML private CenaPadrao cenaController;
    private Tabela<AgendaTipo> tabela = new Tabela<>();
    private ObservableList<AgendaTipo> itens;
    private TableView<AgendaTipo> tabelaLista = new TableView<>();
    ...
    public void acaoFiltrar(Boolean animacao){
        tabelaLista.setItems(new AgendaTipo("", cenaController.getFiltro().getText()).listar());
        if(animacao){
    ...
    public void acaoCadastrar(int botao) {
        AgendaTipoFormularioControlador Controlador = Janela.abrir(AgendaTipo.FXML_FORMULARIO, TITULO);
    ...
    public void acaoAjuda() {
        ...
        Ajuda.getInstance().capitulo(Posicao.CENTRO, idioma.getMensagem("tuto_agenda_tipo_1"));
